// JPG Data Vehicle (jdvrif v7.3) Created by Nicholas Cleasby (@CleasbyCode) 10/04/2023
...
To correctly download images from X-Twitter or Reddit, click image within the post to fully expand it before saving.
    ...
    void optimizeImage(vBytes& jpg_vec, bool isProgressive) {
        ...
        // Check quality AFTER metadata/thumbnail removal to get the main image's quality
    ...
    void segmentDataFile(vBytes& segment_vec, vBytes& data_vec, vBytes& jpg_vec, vString& platforms_vec, bool hasRedditOption) {
        ...
        if (profile_with_data_vec_size > max_first_segment_size) {
            ...
            updateValue(segment_vec, PROFILE_SIZE_INDEX, PROFILE_SEGMENT_SIZE);
    ...
    [[nodiscard]] std::size_t encryptDataFile(vBytes& segment_vec, vBytes& data_vec, vBytes& jpg_vec, vString& platforms_vec, std::string& data_filename, bool hasBlueskyOption, bool hasRedditOption) {
        const std::size_t
    ...
    void zlibFunc(vBytes& data_vec, Mode mode) {
        ...
        if (mode == Mode::conceal) {
            ...
            const int COMPRESSION_LEVEL = (INPUT_SIZE > THRESHOLD_BEST_SPEED)  ? Z_BEST_SPEED :
                                          (INPUT_SIZE > THRESHOLD_DEFAULT)     ? Z_DEFAULT_COMPRESSION : Z_BEST_COMPRESSION;
    ...
    int concealData(vBytes& jpg_vec, Mode mode, Option option, fs::path& data_file_path) {
        ...
        isCompressedFile = data_size > 10 * 1024 * 1024 && hasFileExtension(data_file_path, {
    ...
    int recoverData(vBytes& jpg_vec, Mode mode, fs::path& image_file_path) {
        constexpr std::size_t
            SIG_LENGTH = 7,
            INDEX_DIFF = 8;
